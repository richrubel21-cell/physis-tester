...
export default function EcosystemApproveModal({ run, onClose }) {
  const planned = run?.apps_planned_json || [];
  const [name, setName]               = useState(run?.business_description?.slice(0, 60) || "");
  const [tagline, setTagline]         = useState("");
  const [description, setDescription] = useState(run?.business_description || "");
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${PHYSIS_API}/api/marketplace/ecosystems/create`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          app_ids:     planned.map(a => a.name || a.id).filter(Boolean),
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
          <h2 style={{ color: "#cdd6f4", fontSize: "18px", margin: 0 }}>✓ Approve Ecosystem for Marketplace</h2>
        ...
        {success ? (
          <div style={{ padding: "40px 8px", textAlign: "center" }}>
            ...
            <div style={{ color: "#a6e3a1", fontSize: "16px", fontWeight: "700", marginBottom: "8px" }}>
              Ecosystem added to marketplace!
            </div>
          ...
          <>
            <div style={{ marginBottom: "14px" }}>
              <label style={label}>Ecosystem Name</label>
            ...
            <div style={{ marginBottom: "14px" }}>
              ...
              <input type="text" style={input} value={tagsText} onChange={e => setTagsText(e.target.value)} placeholder="productivity, ai, analytics" />
            </div>
            <div style={{ marginBottom: "14px" }}>
              <label style={label}>Included Apps ({planned.length})</label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {planned.map((a, i) => (
                  <span key={i} style={{ background: "#11111b", border: "1px solid #313244", color: "#94e2d5", borderRadius: "14px", padding: "3px 10px", fontSize: "11px" }}>
                    {a.name || `App ${i + 1}`}
                  </span>
                ))}
              </div>
            ...
            <div style={{ marginBottom: "14px", display: "flex", alignItems: "center", gap: "10px" }}>
              <input type="checkbox" id="eco-featured" checked={featured} onChange={e => setFeatured(e.target.checked)} />
              <label htmlFor="eco-featured" style={{ color: "#cdd6f4", fontSize: "13px", cursor: "pointer" }}>Featured ecosystem</label>
            </div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button type="button" onClick={handleSubmit} disabled={submitting} style={{ flex: 2, background: submitting ? "#44385a" : "#7c3aed", color: "white", border: "none", borderRadius: "8px", padding: "10px", fontSize: "13px", fontWeight: "700", cursor: submitting ? "not-allowed" : "pointer" }}>
                {submitting ? "Submitting..." : "Approve Ecosystem"}
              </button>
            </div>
          </>
        )}
